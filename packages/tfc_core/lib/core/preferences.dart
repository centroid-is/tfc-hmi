...
class Preferences implements PreferencesApi {
  ...
  final FilePreferences sharedPreferences;
  final MySecureStorage secureStorage;
  final StreamController<String> _onPreferencesChanged =
      StreamController<String>.broadcast();

  Preferences({required this.database, required this.secureStorage, required this.sharedPreferences});

  static Future<Preferences> create({required Database? db}) async {
    final secureStorage = SecureStorage.getInstance();
    final filePrefs = await FilePreferences.getInstance();
    try {
      if (db == null) {
        return Preferences(database: null, secureStorage: secureStorage, sharedPreferences: filePrefs);
      }
      final prefs = Preferences(database: db, secureStorage: secureStorage, sharedPreferences: filePrefs);
      await prefs.loadFromPostgres();
      return prefs;
    } on PreferencesException catch (e) {
      stderr.writeln(e.message);
      return Preferences(database: db, secureStorage: secureStorage, sharedPreferences: filePrefs);
    }
  }
...
/// A wrapper around FilePreferences that implements PreferencesApi
class FilePreferencesWrapper implements PreferencesApi {
  final FilePreferences _prefs;

  FilePreferencesWrapper(this._prefs);

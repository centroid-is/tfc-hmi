...
void main() {
  ...
  group('MQTT integration', () {
    ...
    test('WebSocket connect to ws://localhost:9001/mqtt, subscribe and receive',
        () async {
      // Skip if WebSocket listener is not available (e.g. Windows Mosquitto
      // binary lacks libwebsockets support).
      final wsAvailable = await isPortListening(9001);
      if (!wsAvailable) {
        print('WebSocket port 9001 not listening — skipping test');
        return;
      }

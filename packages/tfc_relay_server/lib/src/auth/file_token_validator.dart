...
abstract interface class RevocableTokenValidator implements TokenValidator {
  ...
  /// Whether [identity] is still exactly what the current credential set says
  /// about that station.
  ///
  /// False when the station's token is gone, when it now maps to a different
  /// station, and when the station's role has been narrowed — a demotion that
  /// only took effect on the next reconnect would be a demotion an operator
  /// could postpone indefinitely by not reconnecting.
  bool stillValid(Identity identity);
}
...
final class FileTokenValidator implements RevocableTokenValidator {
  ...
  @override
  bool stillValid(Identity identity) =>
      _set.byStation[identity.stationId] == identity;
  ...
  Future<TokenVerdict> validate(HelloParams params) async {
    ...
    final identity = _set.lookup(token);
    if (identity == null) {
...
final class _TokenSet {
  ...
  /// The presented [token] resolved to an identity, or null.
  ///

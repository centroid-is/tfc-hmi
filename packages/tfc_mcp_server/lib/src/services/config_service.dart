  ...
  Future<Map<String, dynamic>?> getAlarmConfig(String uid) {
    return _alarmConfigCache.getOrCompute(uid, () async {
      ...
      return {
        ...
        // Where the alarm sits in the tree. Projected even though no tool
        // edits it, because an update proposal rebuilds the alarm from this
        // map -- leaving these out would quietly move the alarm back to the
        // root the first time anyone edited its title.
        'path': alarm['path'] is List
            ? (alarm['path'] as List).whereType<String>().toList()
            : const <String>[],
        'bindToPath': alarm['bindToPath'] as bool? ?? false,
      };
    });
  }

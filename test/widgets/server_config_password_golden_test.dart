/// Golden image of the OPC-UA server card's credential fields, for design
/// review.
///
/// The server under test has a stored 28-character password. What is under
/// review is that the expanded card gives away nothing about that password —
/// not even its length. `goldens/server_config_password.before-fix.png` is
/// the same card before the fix, where the password field pre-filled the
/// stored secret and its obscuring dots counted out the length to anyone
...
///
/// To update: flutter test test/widgets/server_config_password_golden_test.dart --update-goldens --run-skipped
@Tags(['golden'])
library;
...
StateManConfig _configWithCredentials() => StateManConfig(
      opcua: [
        OpcUAConfig()
...
void main() {
  ...
  // Material icons — the lock on the password field being central to this
  // golden — and the test environment registers neither. Without this the
  ...
  group('server credentials golden',
    ...
    testWidgets('expanded card with a stored password', (tester) async {
      ...
      await pumpAndLoad(
        tester,
        buildTestableServerConfig(stateManConfig: _configWithCredentials()),
      );
      ...
      await expectLater(
        ...
        matchesGoldenFile('goldens/server_config_password.png'),
      );
    });
  });
}

/// Golden image of the OPC-UA server card's Timing section, for design
/// review.
///
/// Under review: the two knobs read as what they do to the plant — how fast
/// the screen updates, and how often the session is renewed — with their
/// accepted ranges on the helper line, rather than as the OPC-UA service
/// parameters they map to. The lifetime is entered in seconds even though it
/// is stored in milliseconds, so `60` is a minute.
///
/// The server shows "Not active" because the test harness has no StateMan —
/// there is no live connection to report in a widget test.
///
/// To update: flutter test test/widgets/server_config_timing_golden_test.dart --update-goldens --run-skipped
@Tags(['golden'])
library;
...
const Size _surface = Size(760, 1500);

StateManConfig _configWithTiming() => StateManConfig(
      opcua: [
        OpcUAConfig()
          ..endpoint = 'opc.tcp://10.104.20.11:4840'
          ..serverAlias = 'ST101'
          ..username = 'operator'
          // Not the defaults: a card showing 100/60 could be rendering
          // hardcoded text and look identical.
...
void main() {
  ...
  // Material icons — the stopwatch heading the Timing section and the gauge
  // and clock on its two fields — and the test environment registers
  // neither. Without this the
  // image comes out as rows of tofu boxes.
  setUpAll(() async {
  ...
  group('server timing golden',
      skip: !Platform.isMacOS ? 'Golden tests only run on macOS' : null, () {
    testWidgets('expanded card showing the Timing section', (tester) async {
      ...
      await pumpAndLoad(
        tester,
        buildTestableServerConfig(stateManConfig: _configWithTiming()),
      );
      ...
      await expectLater(
        ...
        matchesGoldenFile('goldens/server_config_timing.png'),
      );
    });
  });
}

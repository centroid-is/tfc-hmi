/// Golden images of the transport mode switch, for design review.
///
/// Two states, because they are the two things an operator sees. Expanded on a
/// direct-mode station: the switch itself, above the four sections it governs,
/// unchanged. And a station already pointed at a gateway: the card opens on
/// its own settings and the four sections are gone, replaced by a line saying
/// which decision removed them.
///
/// What is under review is that the card reads as a *switch* and not as a
/// fifth section — it is collapsed by default on the direct-mode stations that
/// are the whole plant today, and the page below it is exactly where it was.
///
/// To update: flutter test test/widgets/server_config_transport_golden_test.dart --update-goldens
@Tags(['golden'])
library;
...
void main() {
  ...
  group('transport mode golden',
    ...
    Future<void> pumpPage(WidgetTester tester,
        {PreferencesApi? localPreferences}) async {
    ...
    testWidgets('direct station, card opened', (tester) async {
      ...
      await expectLater(
        ...
        matchesGoldenFile('goldens/server_config_transport_direct.png'),
      );
    });

    testWidgets('gateway station, the four sections are gone', (tester) async {
      ...
      await pumpPage(tester, localPreferences: local);

      await expectLater(
        find.byType(MaterialApp),
        matchesGoldenFile('goldens/server_config_transport_gateway.png'),
      );
    });
  });
}

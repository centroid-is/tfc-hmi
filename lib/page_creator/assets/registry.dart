...
class AssetRegistry {
  ...
  ///
  /// Why this exists: the Type-keyed map above uses `Type.toString()` to match
  /// against the `asset_name` string in page-editor JSON. On native (VM),
  /// `LEDConfig.toString()` returns `"LEDConfig"`. But dart2js minifies type
  /// names in release builds, so `Type.toString()` returns something like
  /// `"minified:ab"` — breaking all asset lookups on web.
  ///
  /// This map duplicates every entry with explicit string keys to avoid
  /// relying on `Type.toString()`.
  ///
  /// TODO: eliminate this duplication by adding a static `typeName` getter
  /// to BaseAsset and building both maps from a single registration list:
  ///   static final _entries = <({String name, Asset Function(Map<String, dynamic>) fromJson, Asset Function() preview})>[
  ///     (name: 'LEDConfig', fromJson: LEDConfig.fromJson, preview: LEDConfig.preview),
  ///     ...
  ///   ];
  /// Then derive _fromJsonFactories, _namedFromJsonFactories, defaultFactories,
  /// and _namedDefaultFactories from _entries.

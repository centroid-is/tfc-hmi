...
/// does not need them), with a once-per-process warning.
///
...
class MacOsMigratingSecureStorage implements MySecureStorage {
  ...
  /// Warn about missing keychain entitlements only once per process — the
  /// fallback fires on every keychain call, so per-call logging would flood
  /// the log.
  static bool _warnedMissingEntitlements = false;
  static void _warnMissingEntitlements(String operation, Object error) {
    if (_warnedMissingEntitlements) return;
    _warnedMissingEntitlements = true;
    _logger.w(
        'CentroidX keychain $operation failed — this build is probably '
  ...
  Future<String?> read({required String key}) async {
    ...
    // error propagates here as well: it could be hiding real pre-migration
    // data, so it must not read as "key absent".
    ...
    if (legacyValue != null) {
      try {
        ...
        // legacy value is still the answer.

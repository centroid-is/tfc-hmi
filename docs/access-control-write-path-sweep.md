...
| `lib/pages/access_session_section.dart:118` | `prefs.setInt(kAccessInactivityMinutesPrefKey, ...)` | device-local | the Session card on `/advanced/access` | `route-gated (Phase 2)` — `users`; and the card records its own audit row through `RefAuditSink`, because a device-local write bypasses `GuardedPreferences` and the width of the elevation window must not change without a row. Input bounded 1..480 before the write; the provider's clamp stays as the backstop for hand-edited stores |
